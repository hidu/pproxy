...
//	}
	if(req["form"]){
		...
		for ( var k in req["form"]) {
			html+="<tr><th width='80px'>"+k+":</th><td>"+req["form"][k].join("<br/>")+"</td></tr>";
		}
		html +="</table></td></tr>"
    }
 ...
 if(res){
	html += "<tr><th width='80px'>content-length:</th><td>"+res["content_length"]+"</td></tr>"
	html += "<tr><th>status:</th><td>" + res["status"] + "</td></tr>";
	
	var body_str=Base64.decode(res["body"])
	var isImg=res["header"]["Content-Type"]!=undefined && res["header"]["Content-Type"][0].indexOf("image")>-1;
	
	var isStatusOk=res["status"]==200
	
	if(!isImg || res["body"].length<1000|| !isStatusOk){
		html += "<tr><th>body:</th><td><pre>" + h(body_str) + "</pre></td></tr>";
	}
	try{
   ...
   if(isImg){
	   html += "<tr><th>body_img:</th><td><img src='data:"+res["header"]["Content-Type"][0]+";base64,"+res["body"] +"'/></td></tr>";  
   }
